...
const oDv = document.createElement('div');
let dvHdr = document.createElement('div');
let dvBdy = document.createElement('div');
let windowlock;
...
function scanBO(curNode) {
  if (checkElemBO(curNode.title)) {
    ...
    curNode.IEbugfix = getParam('hideselects', curNode.title) == 'on';
    ...
    curNode.windowLock = getParam('windowlock', curNode.title).toLowerCase() != 'off';
    curNode.title = '';
    curNode.hasbox = 1;
...
function getParam(param, list) {
  const reg = new RegExp(`([^a-zA-Z]${param}|^${param})\\s*=\\s*\\[\\s*(((\\[\\[)|(\\]\\])|([^\\]\\[]))*)\\s*\\]`);
  const res = reg.exec(list);
  let returnvar;
