 ...
 */
  (function(window, undefined) {
    ...
    const pseudos = `:(${characterEncoding})(?:\\(((['"])((?:\\\\.|[^\\\\])*?)\\3|((?:\\\\.|[^\\\\()[\\]]|${attributes.replace(
      3,
      8
    )})*)|.*)\\)|)`;
    ...
    const rsibling = new RegExp(`${whitespace}*[+~]`);
    ...
    Sizzle.attr = function(elem, name) {
      ...
      let val = fn && hasOwn.call(Expr.attrHandle, name.toLowerCase()) ? fn(elem, name, !documentIsHTML) : undefined;
    ...
    function matcherFromGroupMatchers(elementMatchers, setMatchers) {
      ...
      const superMatcher = function(seed, context, xml, results, expandContext) {
        ...
        const elems = seed || (byElement && Expr.find.TAG('*', (expandContext && context.parentNode) || context));
 ...
 */
  jQuery.event = {
    ...
    trigger(event, data, elem, onlyHandlers) {
      ...
      let namespaces = core_hasOwn.call(event, 'namespace') ? event.namespace.split('.') : [];
  ...
  jQuery.fn.extend({
    ...
    closest(selectors, context) {
      ...
      const pos =
        rneedsContext.test(selectors) || typeof selectors !== 'string' ? jQuery(selectors, context || this.context) : 0;
  ...
  jQuery.fn.extend({
    ...
    replaceWith() {
      const // Snapshot the DOM in case .domManip sweeps something relevant into its fragment
  ...
  if (window.getComputedStyle) {
    ...
    curCSS = function(elem, name, _computed) {
      ...
      let ret = computed ? computed.getPropertyValue(name) || computed[name] : undefined;
  ...
  const tweeners = {
    '*': [
      function(prop, value) {
        ...
        let start = (jQuery.cssNumber[prop] || (unit !== 'px' && +target)) && rfxnum.exec(jQuery.css(tween.elem, prop));
