...
Accordion.prototype = {
  initialize(elem, clickableEntity, checkAllow) {
    this.container = $(elem);
    this.checkAllow = checkAllow || false;
    this.disallowAccessToNextSections = false;
    this.sections = $$(`#${elem} .section`);
    this.currentSection = false;
    const headers = $$(`#${elem} .section ${clickableEntity}`);
    headers.each(header => {
  ...
  openSection(section) {
    ...
    if (section.id != this.currentSection) {
      ...
      const contents = Element.select(section, '.a-item');
      contents[0].show();
  ...
  openNextSection(setAllow) {
    for (section in this.sections) {
      const nextIndex = parseInt(section) + 1;
  ...
  openPrevSection(setAllow) {
    for (section in this.sections) {
      const prevIndex = parseInt(section) - 1;
