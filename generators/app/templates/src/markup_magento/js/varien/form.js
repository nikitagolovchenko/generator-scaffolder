...
VarienForm.prototype = {
  initialize(formId, firstFieldFocus) {
    this.form = $(formId);
    if (!this.form) {
      return;
    }
  ...
  bindElements() {
    const elements = Form.getElements(this.form);
  ...
  elementOnFocus(event) {
    const element = Event.findElement(event, 'fieldset');
    if (element) {
  ...
  reloadChildren(transport) {
    const data = eval(`(${transport.responseText})`);
  ...
  setDataToChild(data) {
    if (data.length) {
      ...
      var child = $(this.cache[this.currLoader].child);
      if (child) {
        var html = `<input type="text" name="${child.name}" id="${child.id}" class="${child.className}" title="${
          child.title
        }" ${this.extraChildParams}>`;
        Element.insert(child, { before: html });
        Element.remove(child);
...
RegionUpdater.prototype = {
  initialize(countryEl, regionTextEl, regionSelectEl, regions, disableAction, zipEl) {
  ...
  _checkRegionRequired() {
    ...
    elements.each(currentElement => {
      ...
      if (!regionRequired) {
        ...
        if (currentElement.tagName.toLowerCase() == 'select' && currentElement.hasClassName('validate-select')) {
...
ZipUpdater.prototype = {
  ...
  _setPostcodeOptional() {
    ...
    // find label
    const label = $$(`label[for="${this.zipElement.id}"]`)[0];
    if (label != undefined) {
