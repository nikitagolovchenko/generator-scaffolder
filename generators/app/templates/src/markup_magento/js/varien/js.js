...
function parseSidUrl(baseUrl, urlExt) {
  const sidPos = baseUrl.indexOf('/?SID=');
  let sid = '';
...
function formatCurrency(price, format, showPlus) {
  ...
  const requiredPrecision = isNaN((format.requiredPrecision = Math.abs(format.requiredPrecision)))
    ? 2
  ...
  const integerRequired = isNaN((format.integerRequired = Math.abs(format.integerRequired)))
    ? 1
...
// Version 1.0
const isIE = navigator.appVersion.match(/MSIE/) == 'MSIE';

if (!window.Varien) var Varien = new Object();

Varien.showLoading = function() {
  const loader = $('loading-process');
  loader && loader.show();
};
...
Varien.searchForm.prototype = {
  initialize(form, field, emptyText) {
    this.form = $(form);
    this.field = $(field);
...
Varien.Tabs.prototype = {
  ...
  showContent(a) {
    ...
    ul.getElementsBySelector('li', 'ol').each(el => {
      const contents = $(`${el.id}_contents`);
      if (el == li) {
...
Varien.DateElement.prototype = {
  initialize(type, content, required, format) {
    if (type == 'id') {
      // id prefix
...
Varien.DOB.prototype = {
  initialize(selector, required, format) {
    const el = $$(selector)[0];
    const container = {};
