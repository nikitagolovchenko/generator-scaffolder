...
Test.Unit.Runner.prototype = {
  ...
  summary() {
    ...
    return `${(this.options.context ? `${this.options.context}: ` : '') +
      this.tests.length} tests, ${assertions} assertions, ${failures} failures, ${errors} errors`;
  }
};
...
Test.Unit.Assertions.prototype = {
  ...
  assertEnumEqual(expected, actual) {
    ...
    try {
      $A(expected).length == $A(actual).length && expected.zip(actual).all(pair => pair[0] == pair[1])
        ? this.pass()
        : this.fail(`${message}: expected ${Test.Unit.inspect(expected)}, actual ${Test.Unit.inspect(actual)}`);
    } catch (e) {
      this.error(e);
  ...
  assertType(expected, actual) {
    ...
    try {
      actual.constructor == expected
        ? this.pass()
        : this.fail(`${message}: expected "${Test.Unit.inspect(expected)}", actual "${actual.constructor}"`);
    } catch (e) {
      this.error(e);
    }
  },
  ...
  assertRespondsTo(method, obj) {
    ...
    try {
      obj[method] && typeof obj[method] === 'function'
        ? this.pass()
  ...
  assertElementsMatch() {
    const expressions = $A(arguments);

    const elements = $A(expressions.shift());
  ...
  assertNotVisible(element) {
    this.assert(
      ...
      `${Test.Unit.inspect(element)} was not hidden and didn't have a hidden parent either. ${'' || arguments[1]}`
    );
  },
...
Test.setupBDDExtensionMethods = function() {
  ...
  [Array.prototype, String.prototype, Number.prototype, Boolean.prototype].each(p => {
