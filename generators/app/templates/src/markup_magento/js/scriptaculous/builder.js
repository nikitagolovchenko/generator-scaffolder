...
var Builder = {
  ...
  node(elementName) {
    ...
    const parentTag = this.NODEMAP[elementName] || 'div';
    ...
    if (arguments[1])
      if (this._isStringOrNumber(arguments[1]) || arguments[1] instanceof Array || arguments[1].tagName) {
        ...
        const attrs = this._attributes(arguments[1]);
        if (attrs.length) {
          try {
            // prevent IE "feature": http://dev.rubyonrails.org/ticket/2707
  ...
  _attributes(attributes) {
    ...
    for (attribute in attributes)
      attrs.push(
        `${attribute in this.ATTR_MAP ? this.ATTR_MAP[attribute] : attribute}="${attributes[attribute]
          .toString()
          .escapeHTML()
          .gsub(/"/, '&quot;')}"`
      );
  ...
  dump(scope) {
    ...
    tags.each(tag => {
      scope[tag] = function() {
        return Builder.node(...[tag].concat($A(arguments)));
      };
    });
  }
};
