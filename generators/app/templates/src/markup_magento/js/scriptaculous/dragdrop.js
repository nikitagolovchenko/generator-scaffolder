...
var Sortable = {
  ...
  mark: function(dropon, position) {
    ...
    if (position == 'after')
      if (sortable.overlap == 'horizontal') Sortable._marker.setStyle({ left: offsets[0] + dropon.clientWidth + 'px' });
    ...
    if (options.tree) {
      return Sortable.tree(element, arguments[1])
        .children.map(function(item) {
          return [name + Sortable._constructIndex(item) + '[id]=' + encodeURIComponent(item.id)].concat(
