...
Control.Slider = Class.create({
  initialize(handle, track, options) {
    ...
    this.handles.each((h, i) => {
      ...
      slider.setValue(
        parseFloat(
          (Object.isArray(slider.options.sliderValue) ? slider.options.sliderValue[i] : slider.options.sliderValue) ||
            slider.range.start
        ),
        i
      );
  ...
  getRange(range) {
    const v = this.values.sortBy(Prototype.K);
    range = range || 0;
  ...
  drawSpans() {
    const slider = this;
    if (this.spans)
      $R(0, this.spans.length - 1).each(r => {
  ...
  draw(event) {
    const pointer = [Event.pointerX(event), Event.pointerY(event)];
    const offsets = Position.cumulativeOffset(this.track);
