...
$j(document).ready(() => {
  ...
  nav
    ...
    .on(pointerEvent, function(event) {
      ...
      const alreadyExpanded = elem.hasClass('menu-active');
  ...
  nav.find('li.level1 a.has-children').on('click', function(e) {
    ...
    const isSubMenuActive = elem.hasClass('sub-menu-active') ? 1 : 0;
  ...
  if ($j('.products-grid').length) {
    ...
    $j(window).on('delayed-resize', (e, resizeEvent) => {
      alignProductGridActions();
    });
  }
